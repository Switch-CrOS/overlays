...
update_ebuild() {
  ...
  sed -i s/"${old_base}"/"${base}"/g chromeos-kernel-5_10-0.0."${version}".ebuild
  sed -i s/"${old_tree}"/"${tree}"/g chromeos-kernel-5_10-0.0."${version}".ebuild
  mv chromeos-kernel-5_10-0.0."${version}".ebuild \
    chromeos-kernel-5_10-0.0."$((version + 1))".ebuild
...
cherry: update ebuild with private patch for kernel 5.10

Apply ${tot_name} #${patchset} (CL:${patch_id}/${patchset}, ${tot_commit_id}) on commit ${base:0:13}.

git fetch cros

git merge-base cros/chromeos-5.10 ${tot_commit_id}
> ${base}
...
diff generated with
git diff --full-index ${base} ${tot_commit_id} > cherry-tot.patch
by running \`sh repack.sh $1\`

BUG=b:185092438
TEST=cros-workon-cherry stop chromeos-kernel-5_10
     emerge-cherry chromeos-kernel-5_10
"
END

  git add chromeos-kernel-5_10-0.0."${version}".ebuild
  git add chromeos-kernel-5_10-0.0."$((version + 1))".ebuild
  git add files/cherry-tot.patch
...
kernel_dir="${script%overlays*}third_party/kernel/v5.10"
...
info "Please run \`cros-workon-cherry stop chromeos-kernel-5_10 &&"\
  "emerge-cherry chromeos-kernel-5_10\` in chroot to verify the result."
